...
import { LogContextPlugin, InputContextPlugin, EnvContextPlugin } from "../..";
...
describe("contexts.plugins.executors", () => {
  ...
  test("add plugin should usable with use()", () => {
    expect(context.has("exec")).toEqual(true);
  ...
  test("command should not call when dry-run enabled", async () => {
    const environment = {
      DRYRUN: "true",
    };

    await mockEnvironment(environment, async () => {
      const ctx = ContextBuilder.fromContext(context).build();
