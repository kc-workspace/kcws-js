...
describe("contexts.plugins.loggers", () => {
  ...
  const context = ContextBuilder.empty().addPlugin(log).build();
  ...
  test.each([
    ...
    ["{0} {1}", [undefined, undefined] as any, "<undefined> <undefined>"],
    ["{a}", [{ a: undefined }] as any, "<undefined>"],
  ...
  test("group message together", async () => {
    await context.use("log").group("hello", () => {
  ...
  test("setFailed when run throw", () => {
    const error = new Error("hello world");
    context.use("log").throw(error);
